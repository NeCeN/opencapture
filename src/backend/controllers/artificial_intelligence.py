...
def retrieve_documents():
    ...
    _docservers = _vars[9]
    data = []
    for file_name in os.listdir(_docservers.get('TRAIN_PATH_FILES')):
...
def get_model_by_id(model_id):
    ...
    if error is None:
        ...
        response = {
            "errors": gettext('GET_MODEL_BY_ID_ERROR'),
            "message": error
        }
        return response, 401
        ...
        if dir_name in chosen_files:
            ...
            for file_name in os.listdir(file_path + "/" + dir_name):
                if file_name.lower().endswith(tuple(image_format)):
                    ...
                    rows.append(line)
                    print(file_name, ": done (" + str(i), "out of", str(fold_length) +
                          "; folder", str(j) + "/" + str(len(chosen_files)) + ")")
...
def launch_pred(mname, data):
    ...
    if os.path.exists(model_name):
        ...
        error = gettext('MODEL_DOESNT_EXISTS')
        response = {
            "errors": gettext('GET_MODEL_BY_ID_ERROR'),
            "message": error
        }
...
def rename_model(new_name, model_id):
    ...
    model_path = _doc_servers.get('AI_MODEL_PATH')
