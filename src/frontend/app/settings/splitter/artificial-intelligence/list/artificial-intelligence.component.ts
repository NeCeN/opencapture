...
export class ArtificialIntelligenceComponent implements OnInit {
    ...
    deleteOutput(modelId: number) {
        if (modelId !== undefined) {
            this.http.delete(environment['url'] + '/ws/artificial_intelligence/delete/' + modelId, {headers: this.authService.headers}).pipe(
                tap(() => {
                    this.retrieveModels();
    ...
    onSubmit() {
        ...
        if (this.uploadForm.get('file')?.value){
            ...
            this.http.post<any>(environment['url'] + '/ws/artificial_intelligence/testModel/' + disp, formData, {headers: this.authService.headers}).pipe(
